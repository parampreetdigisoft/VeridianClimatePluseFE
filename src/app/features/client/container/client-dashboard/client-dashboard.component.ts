...
export class ClientDashboardComponent implements OnInit, OnDestroy {
  ...
  onKpiChange(data: any): void {
    if (!data) {
      this.toaster.showWarning('No KPI data received.');
      return;
    }
    this.selectedKpiData = data;
    this.clientService.addClientKpisProgramAndPillar(data).subscribe({
      next: (res) => {
        this.closeModal();
        if (res.succeeded) {
          this.toaster.showSuccess('KPI data added successfully.');
        }else {
          this.toaster.showError(res.errors?.[0] || 'Failed to add KPI data.');
        }
        this.getClientPrograms();
      },
      error: (err) => {
        this.closeModal();
        this.toaster.showError('Failed to add KPI data.');
      }
    });
  }
      ...
      next: (res) => {
        ...
        if (res.succeeded) {
          ...
          if (this.programs.length) {
            ...
            this.getAllPillars();
            if (this.tier > 0 && (!this.pillars || this.pillars.length === 0)) {
              this.openDialog();
            }
            // this.openDialog();
          }
