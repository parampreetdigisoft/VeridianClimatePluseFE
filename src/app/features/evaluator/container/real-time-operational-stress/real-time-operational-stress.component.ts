...
export class RealTimeOperationalStressComponent implements OnInit, OnDestroy {
  ...
  loadStressDashboard(): void {
    ...
    this.isLoading = true;
    this.evaluatorService.getPeaceStressTestDashboard(this.selectedclimateProgramID).subscribe({
      next: (res) => {
        this.isLoading = false;
        if (!res.succeeded) {
  ...
  loadEarlyWarningDashboard(isSilent = false): void {
    ...
    this.evaluatorService.getEarlyWarningDashboard(this.selectedclimateProgramID).subscribe({
      next: (res) => {
  ...
  loadResilienceDashboard(): void {
    if (!this.selectedclimateProgramID) return;
    this.isLoading = true;
    this.evaluatorService.getResilienceScorecard(this.selectedclimateProgramID).subscribe({
      next: (res) => {
        this.isLoading = false;
        if (!res.succeeded) {
