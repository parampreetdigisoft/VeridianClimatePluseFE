...
export class AiProgramComparisonComponent implements OnInit {
  ...
  getProgramUserPrograms() {
    this.analystService.getAllProgramsByUserId(this.userService.userInfo.userID).subscribe((p) => {
      this.isLoader = false;
    ...
    const radarOptions: Partial<ChartOptions> = {
      ...
      tooltip: {
        ...
        custom: ({ series, seriesIndex, dataPointIndex, w }) => {
          ...
          const avgScore = cityScores.reduce((sum, program) => sum + program.score, 0) / cityScores.length;
  ...
  getCityScore(climateProgramID: number, isAi: boolean = false): string {
